...
(DETERMINISTIC cuDNN variant).

This variant forces deterministic cuDNN + disables benchmark + disables TF32
to isolate AEROS-implementation-induced numerical residual from cuDNN-
algorithm-selection-induced ULP differences across batch shapes.

Differences from p9_12_dvs_inference.py:
  - torch.backends.cudnn.benchmark = False
  - torch.backends.cudnn.deterministic = True
  - torch.use_deterministic_algorithms(True, warn_only=True)
  - torch.backends.cuda.matmul.allow_tf32 = False
  - torch.backends.cudnn.allow_tf32 = False
  - CUBLAS_WORKSPACE_CONFIG=:4096:8 (env var, set inside script)

If max_abs_err drops to ~1e-7 (fp32 precision floor) or 0, the residual seen
in the auto-tuned run was cuDNN-induced. If it persists at 1e-3 magnitude,
the residual is structural and must be addressed in the AEROS forward path.

Usage:
    python p9_12_dvs_inference_det.py \\
        --ckpt /data/yhr/AEROS/checkpoints_dvs/dvs128_gesture_best.pth \\
        --data_root /data/yhr/AEROS/DVS128Gesture \\
        --output p9_12_dvs_inference_det --kappa 4
"""
...
def main():
    ...
    parser.add_argument("--output", default="p9_12_dvs_inference_det")
    args = parser.parse_args()

    if not torch.cuda.is_available():
        raise RuntimeError("CUDA required")

    # ===== DETERMINISM FLAGS (the only difference from p9_12_dvs_inference.py) =====
    # Required env var for deterministic cuBLAS reductions on Volta+
    os.environ["CUBLAS_WORKSPACE_CONFIG"] = ":4096:8"
    torch.backends.cudnn.benchmark = False
    torch.backends.cudnn.deterministic = True
    # Disable TF32 (Ampere+) — V100 has no TC-TF32 anyway, but be safe
    torch.backends.cuda.matmul.allow_tf32 = False
    torch.backends.cudnn.allow_tf32 = False
    # Force PyTorch to error/warn on any non-deterministic op
    try:
        torch.use_deterministic_algorithms(True, warn_only=True)
    except Exception as e:
        print(f"  [warn] torch.use_deterministic_algorithms not available: {e}")
    print("  [det] cudnn.deterministic=True benchmark=False tf32=False")
    # ============================================================================
    ...
    for name, r in results.items():
        ...
        marker = "✓" if r["max_abs_err_vs_baseline"] < 1e-6 else "✗"
