...
@torch.no_grad()
def run_s4_aeros_segment(net, T, b, C, H, kappa, device, n_repeats=5) -> Cell:
    """AEROS Mode 4 (IO-streaming). No graph capture; per-segment forward."""
    cell = Cell(system="S4_aeros_segment", net="?", T=T, kappa=kappa)
    try:
        reset_gpu(device)
        # Use IO-streaming pattern from Exp 4
        def one_run():
            reset_state_compat(net)
...
def sweep(args, device):
    ...
    runners = {
        ...
        "S3_fullhorizon_cudagraph":  lambda net, T: run_s3_full_horizon_cuda_graph(
            net, T, args.b, 3, args.H, device, args.n_repeats),
        "S4_aeros_segment":          lambda net, T: run_s4_aeros_segment(
    ...
    for name, builder in builders.items():
        ...
        for T in Ts:
            for sys_name, runner in runners.items():
                ...
                cell.T = T
                if cell.system in ("S4_aeros_segment", "S5_aeros_segment_cudagraph"):
                    cell.kappa = kappa
                else:
                    cell.kappa = T
...
def summarize(cells, output_path):
    ...
    systems = ["S1_eager", "S2_torchscript", "S3_fullhorizon_cudagraph",
               "S4_aeros_segment", "S5_aeros_segment_cudagraph"]
