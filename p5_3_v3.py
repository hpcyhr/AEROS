"""
P5-3 v3: corrected ablations (c) and (d).

(c) v3: real generator-based streaming — NO pre-materialization of full x.
        Expected: streaming peak ~constant in T (only segment input held);
        full-input peak grows linearly with T and OOMs at large T.

(d) v3: single-layer LIF integrator with controlled parameters that
        mathematically guarantee non-trivial v at chunk boundaries.
        Setup: τ=20, V_th=10, constant input I=0.6, K=16.
        At t=K=16, v ≈ 6.5 (close to V_th=10 but pre-fire).
        With state-carry: fires at t ≈ 34 (continuing integration).
        Without state-carry: v resets to 0, must integrate from scratch,
        fires at t ≈ 34 from chunk start (= absolute t ≈ 50).
        Spike timing differs by ~16 steps → guaranteed divergence.
...
# =============================================================================
# Ablation (c) v3: full-input vs *real* streaming
# =============================================================================
...
def ablation_c_v3(device):
    print('\n' + '=' * 78)
    print('=== Ablation (c) v3: full-input (pre-materialized) vs '
          'real streaming (generated) ===')
    print('=== at b=32, H=224, K=8 — input alone is 0.0192 GB/step at this regime ===')
    print('=' * 78)
    ...
    full_oomed = False
    for T in Ts:
        # Skip full-input if previous already OOMed
        if full_oomed:
            full_mem = None
            full_str = 'skipped'
        else:
...
# =============================================================================
# Ablation (d) v3: single-layer LIF with controlled integrator params
# =============================================================================
class TinyLIF(nn.Module):
    """Minimal SNN: identity → LIF. State carry/reset is fully visible."""
...
def ablation_d_v3(device):
    print('\n' + '=' * 78)
    print('=== Ablation (d) v3: state-carry necessity ===')
    print('=== TinyLIF (single-layer): τ=20, V_th=10, constant I=0.6 ===')
    print('=== Expected: v at chunk boundary K=16 is ~6.5; reset clobbers this ===')
    ...
    # Constant positive input — every neuron, every step
    I = 0.6
    ...
    if err_carry == 0.0 and err_nocarry > 0.0:
        ...
        print(f'    → Resetting LIF state between chunks DOES change spike output '
              f'in this regime')
    elif err_carry > 0.0:
        print(f'\n  WARN: state-carry DID NOT achieve bit-exact (err = {err_carry:.2e})')
        print(f'    This suggests an implementation issue, not a regime issue.')
    else:
        print(f'\n  WARN: even controlled regime did not show divergence.')
        print(f'    Need to investigate whether reset_net is actually clearing v.')
...
def main():
    ...
    np.savez('p5_3_v3_results.npz', **{f'ablation_{k}': v for k, v in out.items()})
    print('\n  Saved to p5_3_v3_results.npz')
