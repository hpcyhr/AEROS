...
def reset_state(net, provenance="snn"):
    """Provenance-aware state reset. SNN nets use SJ functional.reset_net;
    extended-suite nets use their own reset_state() / submodule reset."""
...
def eval_cell(net_name, T, mode_id, M_budget_GB, coeffs_bundle,
              Delta_resid_bytes, epsilon_safety, device,
              b=32, C=3, H=128, provenance="snn", num_classes=10):
    """One (net, T, mode, M_budget) cell under one solver config.

    The (b, C, H, provenance, num_classes) determine which arch builder is
    used and which input shape is allocated. provenance="snn" → SJ build,
    H typically 128. provenance="extended" → aeros_models_extended,
    H typically 32.
    """
    ...
    # Build via dispatch (handles SNN + extended uniformly)
    if DISPATCH_OK:
    ...
    if mode_id in (3, 4):
        x = torch.randn(T, b, C, H, H, device="cpu", pin_memory=True)
    else:
...
def main():
    ...
    # Load coefficients (supports unified 16-arch bundle, legacy 10-arch
    # SNN-only bundle, or bare-dict legacy)
    if DISPATCH_OK:
    ...
    snn_count = sum(1 for v in bundle["arch_provenance"].values() if v == "snn")
    ext_count = sum(1 for v in bundle["arch_provenance"].values() if v == "extended")
    print(f"    provenance: SNN={snn_count}, extended={ext_count}")
    if "config_snn" in bundle:
        cs = bundle["config_snn"]
        print(f"  SNN config: b={cs.get('b')} C={cs.get('C')} H={cs.get('H')}")
    if "config_extended" in bundle:
        ce = bundle["config_extended"]
        print(f"  extended config: b={ce.get('b')} C={ce.get('C')} H={ce.get('H')}")
    ...
    for idx, (net_name, T, M_GB, mode_id) in enumerate(cells, 1):
        ...
        # Per-arch config (b, C, H, num_classes, provenance)
        cfg = get_arch_config(bundle, net_name) if DISPATCH_OK else {
            "b": args.batch_size, "C": args.C, "H": args.H,
            "num_classes": 10, "provenance": "snn",
        }

        print(f"\n[{idx}/{len(cells)}] {net_name} ({cfg['provenance']}, "
              f"H={cfg['H']}) T={T} mode={mode_id} M={M_GB}GB "
              f"Δ={Delta_resid_bytes/BYTES_PER_GB:.3f}GB")

        # CALIBRATED
        r_cal = eval_cell(net_name, T, mode_id, M_GB, coeffs_bundle,
                          Delta_resid_bytes, args.epsilon_safety,
                          device, b=cfg["b"], C=cfg["C"], H=cfg["H"],
        ...
        r_ne = eval_cell(net_name, T, mode_id, M_GB, coeffs_bundle,
                         0.0, 0.0, device, b=cfg["b"], C=cfg["C"],
                         H=cfg["H"], provenance=cfg["provenance"],
                         num_classes=cfg["num_classes"])
