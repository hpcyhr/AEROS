...
def held_out_envelope(cal_results, delta=0.05):
    """Compute residual envelope from CALIBRATION cell measured residuals
    (measured - predicted, GB). This replaces the in-sample fit-residual
    quantile with a truly held-out one."""
    resids_under_bytes = []
...
def main():
    ...
    for idx, (net_name, T, M_GB, mode_id) in enumerate(eval_cells, 1):
        ...
        print(f"\n[{idx}/{len(eval_cells)}] IN-SAMPLE  {net_name} T={T} "
              f"mode={mode_id} M={M_GB}GB  Δ_in={Delta_in/BYTES_PER_GB:.3f}GB")
    ...
    for idx, (net_name, T, M_GB, mode_id) in enumerate(cal_cells, 1):
        coeffs_arch = coeffs_bundle[net_name]
        Delta_in = in_sample_envelope(coeffs_arch, T, args.delta)
        print(f"\n[{idx}/{len(cal_cells)}] CALIBRATE {net_name} T={T} "
              f"mode={mode_id} M={M_GB}GB")
        r = run_cell_with_envelope(
            net_name, T, mode_id, M_GB, coeffs_bundle,
            Delta_in, args.epsilon_safety, device,
            b=args.batch_size, C=args.C, H=args.H)
    ...
    Delta_held = held_out_envelope(cal_results, args.delta)
    print()
    print(f"  Held-out envelope Δ_held = {Delta_held/BYTES_PER_GB:.4f} GB"
          f" (vs in-sample 0.000-0.140 GB range per arch)")

    # ---- Step 3: rerun evaluation cells using held-out envelope ----
    print()
    print("=" * 78)
    print("Step 3: held-out compliance (use Δ_held in solver)")
    print("=" * 78)
    held_out_results = []
    for idx, (net_name, T, M_GB, mode_id) in enumerate(eval_cells, 1):
        print(f"\n[{idx}/{len(eval_cells)}] HELD-OUT  {net_name} T={T} "
              f"mode={mode_id} M={M_GB}GB  Δ_held={Delta_held/BYTES_PER_GB:.3f}GB")
        r = run_cell_with_envelope(
            net_name, T, mode_id, M_GB, coeffs_bundle,
            Delta_held, args.epsilon_safety, device,
            b=args.batch_size, C=args.C, H=args.H)
    ...
    out = {
        ...
        "Delta_held_GB": Delta_held / BYTES_PER_GB,
